<!--
  ==============================================================================
  ADR-0001 is the inaugural Architecture Decision Record for this baseline.
  It serves three purposes simultaneously:

    1. The authoritative DECISION that ADRs are the documentation format
       used in repositories that adopt this baseline for architecturally
       significant choices.
    2. The canonical TEMPLATE that subsequent ADRs in repositories that
       adopt this baseline are copied from.
    3. The worked EXAMPLE that demonstrates a strong ADR in practice.

  How to use this file as a template:

    1. Copy this file to one of two destinations:
         (a) .github/docs/decision-records/NNNN-short-kebab-title.md in
             a repository that adopts this baseline, to create a new ADR.
             NNNN is the next unused four-digit number in that
             repository's .github/docs/decision-records/ directory,
             allocated monotonically and never reused.
         (b) .github/docs/decision-records/0001-use-architecture-decision-records.md
             in a sibling repository, to seed that repository's initial
             ADR-0001 using this baseline.
    2. Replace the title, the metadata table, and every section body with
       content that is true for the new decision (case a) or for the
       sibling repository (case b). Keep the section headings in the
       order shown.
    3. For sections or metadata fields that have no content, keep them
       ...
       nothing to record."
    4. Open a pull request. When the ADR's state later changes, update its
       Status field and any cross-referenced ADRs whose Supersedes,
       Superseded by, or Implementing PRs fields change over time.

  In case (a), delete this HTML comment block before committing the new
  ADR. In case (b), keep the block so the sibling repository's ADR-0001
  can continue to serve as that repository's template.
  ==============================================================================
-->
...
We will use `.github/docs/decision-records/` as the conventional home for architecturally significant decisions in repositories that adopt this baseline. The format is MADR 4.0-aligned but uses a visible Markdown metadata table, adds explicit reversibility, implementing-PR links, and a conservative compliance-notes crosswalk, and uses the more readable `decision-records` directory name in place of MADR's conventional `adr/`. This gives participating repositories an in-repository decision log that stays close to the code and is more useful for maintenance, review, onboarding, and audit preparation than ad hoc notes, wikis, or unstructured commit history.
...
At the portfolio root, this directory defines the baseline ADR shape that participating repositories can copy when they create their own `.github/docs/decision-records/` directory, their own `ADR-0001`, and later records. Those repositories should reuse the structure, not this prose verbatim; the context, consequences, and compliance notes must still be rewritten so they are true for the repository that adopts them.
...
1. **Presence check.** A repository that adopts this baseline MUST contain `.github/docs/decision-records/0001-use-architecture-decision-records.md`, derived from this template but rewritten anywhere repository-specific context differs. A CI job or `pre-commit` hook MAY fail a pull request that removes or invalidates that file.
2. **Schema check.** In a repository that adopts this baseline, a CI script SHOULD verify that every file matching `.github/docs/decision-records/[0-9][0-9][0-9][0-9]-*.md` contains the required section headings from this template: `## TL;DR`, `## Context and Problem Statement`, `## Decision Drivers`, `## Considered Options`, `## Decision Outcome`, `## Pros and Cons of the Options`, `## Confirmation`, `## Consequences`, `## Assumptions`, `## Supersedes`, `## Superseded by`, `## Implementing PRs`, `## Related ADRs`, and `## Compliance Notes`. `## Considered Options` and `## Pros and Cons of the Options` are especially important because they preserve rejected alternatives and trade-offs.
3. **Index check.** In a repository that adopts this baseline, `.github/docs/decision-records/README.md` MUST list every ADR in the directory with its current Status and Summary. A CI script SHOULD diff the directory listing against the index and fail on drift.
4. **Human review.** Every pull request that introduces a new ADR MUST be reviewed. Every pull request that materially contradicts an Accepted ADR SHOULD either update the code to comply, supersede the ADR, or explain why the ADR never actually applied to the change in question.
5. **Editorial rule.** After acceptance, edits MAY correct typos, broken links, formatting, Status, supersession fields, or `Implementing PRs`, but they MUST NOT silently change the decision, its scope, or its rationale.
...
- In repositories that adopt this baseline, the directory name `.github/docs/decision-records/` and the filename pattern `NNNN-short-kebab-title.md` are established conventions.
- Each adopting repository's `.github/docs/decision-records/README.md` becomes the canonical local index for its ADR set.
- Future repositories may introduce carefully scoped local extensions, but those should be documented in their own ADRs rather than by silently mutating this baseline.
